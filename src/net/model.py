...
def train(model, train_set, val_set, n_epoch, save_file=None, load_file=None):
    ...
    if load_file:
        model.load_params(load_file)
        accuracy = evaluate(model, val_set.images, val_set.labels)
    ...
    while train_set.epochs_completed < n_epoch:
        batch_xs, batch_ys = train_set.next_batch(model.batch_size)

        _, cost_val = model.sess.run([train_op, cost], feed_dict={model.X: batch_xs, model.Y: batch_ys, model._is_training: True})
...
def evaluate(model, images, labels):    
    ...
    correct_prediction = tf.equal(tf.argmax(model.Y_pred, 1), tf.argmax(model.Y, 1))
    accuracy_op = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))
    accuracy = model.sess.run(accuracy_op, feed_dict={model.X: images, model.Y: labels, model._is_training: False})
    return accuracy
