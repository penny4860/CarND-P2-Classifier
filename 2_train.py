...
if __name__ == '__main__':
    ...
    with tf.Session() as sess:
        ...
        while data_train.epochs_completed < n_epoch:
            ...
            _, cost_value = sess.run([train_op, cost_op], feed_dict={x: batch_xs, y: batch_ys})
            # print ("train cost: ", cost_value)

            if epoch != data_train.epochs_completed:
                epoch += 1
                cost_value = sess.run(cost_op, feed_dict={x: data_val.images, y: data_val.labels})
                accuracy = sess.run(accuracy_op, feed_dict={x: data_val.images, y: data_val.labels})
